...
- 自定义Row的cell
  ...
  - 自定义cell要继承自XLFormBaseCell类
    ...
    +(**void**)load 【非必须】可以在这里，在主表单中注册此自定义cell以及对应的ID

    -(**void**)configure【必须】在这里进行UI搭建和配置

    -(**void**)update【必须】外面给控件赋值以后就会调用这个？可以在这里进行赋值后的操作

  - 在创建row时

    必须使用自定义cell的ID(type), 必须设置row的cellClass为自定义cell，否则会出现错误
